...
features_dir = '/storage/che011/ICT/fairseq-image-captioning/output_yolov4/'
...
def extractFeatures(data,
    ...
    if False: #training:  # called by train.py
    ...
    features_task_dir = os.path.join(features_dir, feature_dir_name)
    print(features_task_dir)
    if os.path.isdir(features_task_dir):
        shutil.rmtree(features_task_dir)
    os.makedirs(features_task_dir)
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            if extract_features:
                ...
                path_id = int(os.path.splitext(os.path.basename(paths[0]))[0].replace('COCO_train2014_','').replace('COCO_val2014_',''))
                np.save(os.path.join(features_task_dir,str(path_id)), features.detach().cpu().numpy())
            t0 += time_synchronized() - t

            # # # # # # # Compute loss
            # # # # # # if False: #training:  # if model has loss hyperparameters
        ...
        # # # # # # for si, pred in enumerate(output):
            ...
            # # # # # # xywh = [(xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1) for *xyxy, conf, cls in pred]  # normalized xywh
            # # # # # # xywh_full = torch.cat(xywh, dim=0)
            ...
            # # # # # # if extract_features:
                ...
                # # # # # # features_info = {
                    ...
                    # # # # # # 'image_h': height,
                    # # # # # # 'image_w': width,
                    # # # # # # 'num_boxes' : xywh_full.shape[0],
                    # # # # # # 'boxes': base64.b64encode(xywh_full.detach().cpu().numpy()).decode("utf-8"),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', default='val', help="'val', 'test', 'train'")
    ...
    if opt.task in ['val', 'test', 'train']:  # run normally
        extractFeatures(opt.data,
             opt.weights,
             opt.batch_size,
             opt.img_size,
             opt.conf_thres,
        ...
        for weights in ['']:
            ...
            for i in x:  # img-size
                ...
                r, _, t = extractFeatures(opt.data, weights, opt.batch_size, i, opt.conf_thres, opt.iou_thres, opt.save_json)
